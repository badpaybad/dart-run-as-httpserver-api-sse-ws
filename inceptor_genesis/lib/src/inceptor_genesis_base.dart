...
class RequestData {
  ...
  RequestData(this.path, this.method, this.body, this.replyTo);
}

class OwnerServer {
  HttpServer? _server;

  IpConfing? _ipCfgOwner;

  List<IpConfing> _seederIps = [];
  List<IpConfing> _leecherIps = [];

  void Start(IpConfing ipcfgOwner) async {
    _ipCfgOwner = ipcfgOwner;

    if (ipcfgOwner.ipv6 != "") {
      _server = await HttpServer.bind(
        ipcfgOwner.ipv6 != "" ? ipcfgOwner.ipv6 : InternetAddress.anyIPv6,
        int.parse(ipcfgOwner.port),
      );
    } else {
      _server = await HttpServer.bind(
        ipcfgOwner.ipv4 != "" ? ipcfgOwner.ipv4 : InternetAddress.anyIPv4,
        int.parse(ipcfgOwner.port),
      );
    }
    ...
    await for (HttpRequest request in _server!) {
      ...
      final data = RequestData(
        ...
        request.method,
        body,
        null, // replyTo sẽ thiết lập khi gửi
      );
    }
  }
}
...
class P2PNode {
  ...
  Future<void> startServer() async {
    ...
    print('P2P server running on port $port');
    server.listen(_handleClient);
  }

  void _handleClient(Socket socket) {
    peers.add(socket);
    print('New peer: ${socket.remoteAddress.address}:${socket.remotePort}');
    socket.listen((data) {
  ...
  void connectToPeer(String ip, int port) async {
